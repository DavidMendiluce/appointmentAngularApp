...
@Component({
  ...
  styleUrls: ['./reservation-form.component.css', '../reservation-details/reservation-details.component.css', '../main-page/main-page.component.css'],
  providers: [AppointmentService, LocationService]
})
export class ReservationFormComponent implements OnInit {

  errorMessage: string = "Please enter all the required fields";
  ...
  constructor(calendar: NgbCalendar, private config: NgbDatepickerConfig,
     _router: Router, private _Activatedroute:ActivatedRoute
     , public appointmentService: AppointmentService,public locationsService: LocationService) {

      this.city =this._Activatedroute.snapshot.paramMap.get("location");
      this.roomType =this._Activatedroute.snapshot.paramMap.get("type");
    ...
    console.log("La id en el form es" + this.placeId);
  ...
  changeStatusCarRent() {
    ...
    if(this.checkStatus == false) {
      $('#carRentInput').removeClass('hideCheck');
      $('#carRentInput').addClass('visibleCheck');
      this.checkStatus = true;
    } else {
      $('#carRentInput').removeClass('visibleCheck');
      $('#carRentInput').addClass('hideCheck');
  ...
  changeStatusTaxi() {
    if(this.checkStatus == false) {
      $('#taxiInput').removeClass('hideCheck');
      $('#taxiInput').addClass('visibleCheck');
      this.checkStatus = true;
    } else {
      $('#taxiInputt').removeClass('visibleCheck');
      $('#taxiInput').addClass('hideCheck');
      this.checkStatus = false;
    }
  }
    ...
    onSubmit() {
      ...
      var checkIn: string | string[] | number | undefined | null = this.checkInDate;
      var checkOut: string | string[] | number | undefined | null = this.checkOutDate;
      var nameValue: string | string[] | number | undefined = '';
      var surNameValue: string | string[] | number | undefined = '';
      var emailValue: string | string[] | number | undefined = '';
      var confirmEmailValue: string | string[] | number | undefined = '';
      var phoneValue: string | string[] | number | undefined = '';
      var arrivalTimeValue: string | string[] | number | undefined = '';
      var taxiValue: string | string[] | number | undefined = '';
      var carRentalValue: string | string[] | number | undefined = '';
      var locationValue: string | string[] | number | undefined | null = '';
      var roomTypeValue: string | string[] | number | undefined | null = '';
      var adultGuestVal: string | null= '';
      var childrenGuestVal: string | null = '';
      var priceValue: string | string[] | number | undefined | null = this.locationsService.selectedLocation.price;
      var daysValue: string | string[] | number | undefined | null = '';


      roomTypeValue = this.roomType;

      //form checkbox

      if($('#carRentInput').is(":checked")) {
        carRentalValue = 'Yes';
      } else {
        carRentalValue = 'No';
      }

      if($('#taxiInput').is(":checked")) {
        taxiValue = 'Yes';
      } else {
        taxiValue = 'No';
      }
      ...
      $( "#arrivalInput" )
      .keyup(function() {
       arrivalTimeValue = $( this ).val();
      })
      .keyup();

      var guestString = this.adultGuests + " Adults" + " - " + this.childrenGuests + " Children";
      console.log(guestString);

      console.log(checkIn + " " + checkOut + " " + locationValue + " " + roomTypeValue + " " +
       nameValue + " " +  surNameValue + " " + emailValue + " " + confirmEmailValue + " " + phoneValue + " " +
       arrivalTimeValue + " " + taxiValue + " " + carRentalValue + " ");

       var checkInTest = checkIn!.toString();
          var checkOutTest = checkOut!.toString();

          var checkInDate = new Date(checkInTest);

          var checkOutDate = new Date(checkOutTest);

          const diffTime = Math.abs(checkOutDate.valueOf() - checkInDate.valueOf());
          const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24));

          daysValue = diffDays;


      if(!locationValue.trim() || !checkIn.trim() || !checkOut.trim()
          || !nameValue.trim() || !surNameValue.trim() || !roomTypeValue.trim()
          || !emailValue.trim() || !confirmEmailValue.trim() || !phoneValue.trim()
          || !arrivalTimeValue.trim() || !priceValue
          ){
            $( ".errorBannerWrapper" ).fadeIn( "fast", function() {
                ...
                var apm = {
                  ...
                  days: daysValue,
                  guests: guestString,
                  reservationNumber: Math.floor(Math.random() * (111111111 - 999999999 + 1) + 1),
                  locationId: this.locationsService.selectedLocation.locationId,
                  address: this.locationsService.selectedLocation.address,
                  fromDate: checkIn,
                  toDate: checkOut,
                  propertyType: this.locationsService.selectedLocation.roomType,
                  clientName: nameValue,
                  clientSurname: surNameValue,
                  clientEmail: emailValue,
                  clientPhone: parseInt(phoneValue),
                  arrivalTime: arrivalTimeValue,
                  taxi: taxiValue,
                  carRental: carRentalValue
                }
                this.appointmentService.postAppointment(apm).subscribe((res) => {
                console.log('posted from form component in angular');
              });
          }
  }
