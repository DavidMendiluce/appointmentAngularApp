...
export class MainPageComponent implements OnInit {
  ...
  constructor(calendar: NgbCalendar, private config: NgbDatepickerConfig, _router: Router,
     private _Activatedroute:ActivatedRoute, public authService: AuthService) {
  ...
  ngOnInit(): void {
    ...
    if(this.userId != null) {
      ...
      $('.logoutBtnWrapper').css('display', 'block');
      $('.loginBtnWrapper').css('display', 'none');
      $('.signupBtnWrapper').css('display', 'none');
      $('.loginSignUpPopup').css('height', '5vh');
      $('.loginLineSeparator').css('display', 'none');
      $('.loginWrapper').removeClass('d-flex justify-content-center');
      $('.logoutBtnWrapper').addClass('d-flex flex-column justify-content-center');
    } else {
      console.log('no suser logged');
      $('.logoutBtnWrapper').css('display', 'none');
      $('.loginBtnWrapper').css('display', 'block');
      $('.signupBtnWrapper').css('display', 'block');
      $('.loginSignUpPopup').css('height', '9vh');
      $('.loginLineSeparator').css('display', 'flex');
      $('.loginWrapper').addClass('d-flex justify-content-center');
      $('.logoutBtnWrapper').removeClass('d-flex flex-column justify-content-center');
    }
  ...
  @HostListener('window:scroll', ['$event']) // for window scroll events
    onScroll(event: any) {
      ...
      var barHeight = $(window).width();
      console.log(scrollTop);
      if(scrollTop <= 0) {
        this.setMiniHeader = false;
        setTimeout(function() {
          $('.headerNav').css('box-shadow', '0px 0px 0px #b5b5b5');
        ...
        $('ngb-datepicker').css('top', '22vh');
        $('.dropdownGuests').css('top', '21vh');
        $(window).off('scroll');
        this.scrollFunction();
      } else {
  ...
  searchMain() {
    ...
    if(locationValue == '' || !this.toDate || !this.fromDate ||
        this.numberOfAdults == 0 && this.numberOfChildren == 0){
          $('.errorBannerWrapper').fadeIn( "slow", function() {
            setTimeout(function() {
              $('.errorBannerWrapper').fadeOut("slow");
            }, 4000)
          });
    } else {
    ...
    var formatedCheckOutDate = this.toDate.year + '-' + this.toDate.month + '-' + this.toDate.day;
      ...
      $('.errorBannerWrapper').fadeOut( "slow", function() {
